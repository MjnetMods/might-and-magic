...
public class TestManaPool {
    ...
    /** MP-2: ManaPoolBlockEntity registers in EnergyNetworkHandler after first server tick. */
    @GameTest(template = PLATFORM, timeoutTicks = 20)
    public static void manaPoolRegistersOnPlace(GameTestHelper helper) {
        helper.setBlock(CENTER, VerdantMana.MANA_POOL.get().defaultBlockState());
        helper.runAfterDelay(2, () -> {
            ManaPoolBlockEntity be = MamGameTestHelper.getBlockEntity(helper, CENTER, ManaPoolBlockEntity.class);
            Set<EnergyPool> pools = EnergyNetworkHandler.INSTANCE.getAllPoolsInWorld(helper.getLevel());
            if (!pools.contains(be)) helper.fail("ManaPool not registered in EnergyNetworkHandler after first tick");
            else helper.succeed();
        });
    }

    /** MP-3: removing the pool block removes it from EnergyNetworkHandler (setRemoved fires synchronously). */
    @GameTest(template = PLATFORM, timeoutTicks = 20)
    public static void manaPoolDeregistersOnRemove(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            ...
            if (EnergyNetworkHandler.INSTANCE.getAllPoolsInWorld(helper.getLevel()).contains(be)) {
                helper.fail("ManaPool still in EnergyNetworkHandler after block removal");
            } else {
                helper.succeed();
            }
        });
    ...
    public static void poolTierCapacityIsCorrect(GameTestHelper helper) {
        ...
        for (var c : List.of(
            ...
            ManaPoolBlockEntity be = MamGameTestHelper.getBlockEntity(helper, CENTER, ManaPoolBlockEntity.class);
            if (be.getMaxEnergy() != c.expectedMax()) {
                helper.fail(c.label() + ": expected max " + c.expectedMax() + ", got " + be.getMaxEnergy());
                return;
            }
        }
        helper.succeed();
    ...
    public static void poolTierEnergyTypeIsCorrect(GameTestHelper helper) {
        record Check(Block block, EnergyType expected, String label) {}
        for (var c : List.of(
            new Check(VerdantMana.MANA_POOL.get(), EnergyType.MANA, "mana_pool"),
            new Check(VerdantMana.INFUSED_MANA_POOL.get(), EnergyType.MANA, "infused_mana_pool"),
            new Check(VerdantMana.SACRED_MANA_POOL.get(), EnergyType.MANA, "sacred_mana_pool"),
            new Check(VerdantMana.DESECRATED_MANA_POOL.get(), EnergyType.NOX, "desecrated_mana_pool")
        )) {
            helper.setBlock(CENTER, c.block().defaultBlockState());
            ManaPoolBlockEntity be = MamGameTestHelper.getBlockEntity(helper, CENTER, ManaPoolBlockEntity.class);
    ...
    public static void manaPoolTaintsToNoxOnContact(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            ...
            if (be.getCurrentEnergy() != 1000 || be.getEnergyType() != EnergyType.NOX) {
                helper.fail("Expected 1000 Nox after taint, got " + be.getCurrentEnergy() + " " + be.getEnergyType());
            } else {
                helper.succeed();
            }
        });
    ...
    public static void sacredManaPoolRejectsNox(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            ...
            if (be.getCurrentEnergy() != 0 || be.getEnergyType() != EnergyType.MANA) {
                helper.fail("Expected 0 Mana after rejection, got " + be.getCurrentEnergy() + " " + be.getEnergyType());
            } else {
                helper.succeed();
            }
        });
    ...
    public static void desecratedManaPoolRejectsMana(GameTestHelper helper) {
        ...
        helper.runAfterDelay(2, () -> {
            ...
            if (be.getCurrentEnergy() != 0 || be.getEnergyType() != EnergyType.NOX) {
                helper.fail("Expected 0 Nox after rejection, got " + be.getCurrentEnergy() + " " + be.getEnergyType());
            } else {
                helper.succeed();
            }
        });
