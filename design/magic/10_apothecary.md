...
| Tier                  | Built from             | Secondary ingredient                              | School gate    | Slots         | Notes                                                                                                       |
|-----------------------|------------------------|---------------------------------------------------|----------------|---------------|-------------------------------------------------------------------------------------------------------------|
| Apothecary            | Any rock (`c:stones`)  | `mam:mystical_petals` OR `mam:mystical_mushrooms` | All schools T1 | 4 + 1 (seed)  | Tinted neutral gray (identity tint) — see [[27_tier-tinting]]                                               |
| Infused Apothecary    | Infused Living Rock    | Apothecary                                        | All schools T2 | 6 + 1 (seed)  | Reuses T1's desaturated texture tinted blue via the shared per-tier tint mechanism, see [[27_tier-tinting]] |
| Sacred Apothecary     | Sacred Living Rock     | Infused Apothecary                                | Verdant T3     | 64 + 1 (seed) | Tinted green per [[27_tier-tinting]]                                                                        |
| Desecrated Apothecary | Desecrated Living Rock | Infused Apothecary                                | Dark T3        | 64 + 1 (seed) | Tinted purple per [[27_tier-tinting]]                                                                       |
...
T1's 4-slot capacity matches the bootstrap Pure Daisy recipe ported directly from Botania (`petal_apothecary/pure_daisy.json`): 4 white petals + 1 seed reagent thrown last.
...
- `done` — T1 texture: desaturated own-texture with a neutral gray identity tint — see [[27_tier-tinting]] for the shared mechanism, [manual steps](../../test/09_tier-tinting.md) for visual confirmation
- `done` — Infused Apothecary (T2) recipe — `MamRecipeProvider.infusedApothecary()` (6 Infused
  Living Rock + 1 base Apothecary → `mam:infused_apothecary`), shape-verified by GameTests RC-9
  (ingredient composition) and RC-10 (goblet shape, rejects a same-composition/wrong-position
  layout) in [`TestRecipes`](../../src/main/java/org/mjli/mam/infrastructure/gametest/tests/TestRecipes.java).
  Per-tier ingredient slot capacity (this table's Slots column) is resolved in
  [`ApothecaryBlockEntity`](../../src/main/java/org/mjli/mam/block_entity/ApothecaryBlockEntity.java)
  from the block state, since all tiers share one `BlockEntityType` — regression-guarded by PA-6
  in [`TestApothecary`](../../src/main/java/org/mjli/mam/infrastructure/gametest/tests/TestApothecary.java).
- `todo` — Sacred / Desecrated Apothecary recipes — no recipe methods exist yet in `MamRecipeProvider`
